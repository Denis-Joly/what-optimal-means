"""Write data/stigler_1939.csv from the transcribed 1939 commodity table.

The table is Stigler's own: 77 commodities, August 1939 retail prices averaged
over 51 large cities, and for each commodity the quantity of each of nine
nutrients obtainable for one dollar. The transcription used here is the one
distributed with Google OR-Tools (Apache 2.0), which is itself a transcription
of Stigler's Table 1. It is committed as a CSV so that nothing in this
repository depends on a network fetch, and so that the numbers being solved are
visible rather than buried in a library.

Run once. The CSV it writes is what stigler.py reads.
"""
import csv
import pathlib

# Daily allowances for a moderately active man weighing 154 lb, from the
# National Research Council (1943) as Stigler used them.
NUTRIENTS = [
    ("calories_kcal_thousands", 3.0),
    ("protein_g", 70.0),
    ("calcium_g", 0.8),
    ("iron_mg", 12.0),
    ("vitamin_a_kiu", 5.0),
...
# name, unit, 1939 price in cents for that unit, then the nine nutrients
# obtainable for ONE DOLLAR spent on that commodity.
DATA = [
...
def main():
    assert len(DATA) == 77, f"expected 77 commodities, got {len(DATA)}"
    for row in DATA:
        assert len(row) == 3 + len(NUTRIENTS), f"bad row width: {row[0]}"

    out = HERE / "data" / "stigler_1939.csv"
    out.parent.mkdir(exist_ok=True)
    with out.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["commodity", "unit", "price_cents_aug1939"]
                   + [n for n, _ in NUTRIENTS])
        w.writerows(DATA)

    req = HERE / "data" / "daily_allowances_1943.csv"
    with req.open("w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["nutrient", "daily_minimum"])
        w.writerows(NUTRIENTS)

    print(f"wrote {out} ({len(DATA)} commodities)")
    print(f"wrote {req} ({len(NUTRIENTS)} nutrients)")
